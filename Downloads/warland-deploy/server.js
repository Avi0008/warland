...
const PASS_GO = 200;
const PROPERTY_RENT_MULTIPLIERS = [1, 2, 4, 8, 24, 48];

function rentScheduleFromBase(baseRent) {
  const rent = Math.max(0, Math.floor(Number(baseRent) || 0));
  return PROPERTY_RENT_MULTIPLIERS.map((multiplier) => rent * multiplier);
}

function houseCostFromBaseRent(baseRent) {
  const rent = Math.max(0, Math.floor(Number(baseRent) || 0));
  if (rent <= 10) return 50;
  if (rent <= 15) return 100;
  if (rent <= 20) return 150;
  return 200;
}
...
const BOARD = [
  ...
  { name: "City Tax", type: "tax", taxRate: 0.1, taxLabel: "10% net worth" },
  ...
  const normalized = { ...tile, id: index };
  if (normalized.type !== "property") return normalized;
  const baseRent = normalized.rent?.[0] || 0;
  return {
    ...normalized,
    rent: rentScheduleFromBase(baseRent),
    houseCost: houseCostFromBaseRent(baseRent)
  };
});
...
function generateRoomName() {
  const openers = ["Rent", "Auction", "Hotel", "Jailbreak", "Tax", "Skyline", "Victory", "Cash"];
  const moods = ["Riot", "Fever", "Heist", "Storm", "Circuit", "Flex", "Rush", "Showdown"];
  const closers = ["HQ", "Club", "Arena", "Table", "District", "League", "Squad", "Zone"];
  return `${randomFrom(openers)} ${randomFrom(moods)} ${randomFrom(closers)}`;
}
  ...
  try {
    ...
    for (const room of data.rooms || []) {
      ...
      rooms.set(room.id, {
        ...
        startedPlayerCount: room.startedPlayerCount || (room.status === "playing" ? room.players.length : 0),
      ...
      if (room.auction) scheduleAuctionTimer(room);
    }
  } catch (error) {
...
function playerPublic(player, room) {
  return {
    ...
    previousPosition: player.previousPosition ?? player.position,
    movementPath: player.movementPath || [],
    jailTurns: player.jailTurns,
    bankrupt: player.bankrupt,
    jailCards: player.jailCards,
    lastRoll: player.lastRoll,
    netWorth: playerNetWorth(room, player)
  };
}
...
function publicRoom(room) {
  ...
  return {
    ...
    players: room.players.map((player) => playerPublic(player, room)),
    board: BOARD,
...
function createRoom(name, maxPlayers = 6) {
  const room = {
    id: id("R"),
    name: cleanText(name, generateRoomName(), 40),
...
function availableColor(room, requestedColor) {
  const taken = new Set(room.players.map((player) => player.color));
  if (requestedColor && COLORS.includes(requestedColor) && !taken.has(requestedColor)) return requestedColor;
  return COLORS.find((color) => !taken.has(color)) || COLORS[room.players.length % COLORS.length];
}

function addPlayer(room, name, requestedColor = "") {
  const player = {
    id: id("P"),
    name: cleanText(name, `Player ${room.players.length + 1}`, 24),
    color: availableColor(room, requestedColor),
...
function addEvent(room, kind, text, extra = {}) {
  room.lastEvent = { id: id("E"), kind, text, at: Date.now(), ...extra };
  addLog(room, text, kind);
}
...
function activePlayer(room, playerId) {
  ...
  if (player.bankrupt) throw new Error("You are bankrupt and cannot take turns.");
  return player;
}
...
function nextPlayer(room) {
  ...
  if (contenders.length === 0) {
    room.status = "ended";
    room.winnerId = null;
    room.turnMessage = "No winner.";
    addLog(room, room.turnMessage, "win");
    return;
  }
  if (contenders.length === 1 && (room.startedPlayerCount || room.players.length) > 1) {
    room.status = "ended";
    room.winnerId = contenders[0].id;
    room.turnMessage = `${contenders[0].name} wins the table.`;
    addLog(room, room.turnMessage, "win");
    return;
  }
  if (contenders.length === 1) {
    const solo = contenders[0];
    room.activePlayerId = solo.id;
    room.pendingPurchase = null;
    room.doubles = 0;
    room.turnMessage = `${solo.name}'s turn.`;
    return;
  }
...
function maybeAutoAdvanceTurn(room) {
  ...
  if (room.pendingPurchase || room.auction) return;
  const player = room.players.find((item) => item.id === room.activePlayerId);
  if (!player || player.bankrupt) return;
  if (player.cash < 0) {
    room.turnMessage = `${player.name} must clear ${moneyValue(Math.abs(player.cash))} debt before the turn can end.`;
    return;
  }
  if (room.currentRoll && !room.currentRoll.canRollAgain) nextPlayer(room);
}
...
function playerNetWorth(room, player) {
  if (!room || !player) return 0;
  const assetValue = playerAssets(room, player.id).reduce((total, { tile, prop }) => {
    const baseValue = prop.mortgaged ? Math.floor(tile.price / 2) : tile.price;
    const buildingValue = tile.houseCost ? (prop.houses || 0) * tile.houseCost : 0;
    return total + baseValue + buildingValue;
  }, 0);
  return Math.max(0, Math.floor(player.cash + assetValue));
}

function taxAmountFor(room, player, tile) {
  if (tile.taxRate) {
    return Math.max(0, Math.round((playerNetWorth(room, player) * tile.taxRate) / 10) * 10);
  }
...
function movementPath(from, to) {
  const path = [];
  let cursor = from;
  while (cursor !== to && path.length < BOARD.length) {
...
function setPlayerPosition(player, position) {
  const nextPosition = ((Number(position) % BOARD.length) + BOARD.length) % BOARD.length;
  player.previousPosition = player.position;
  player.position = nextPosition;
  player.movementPath = movementPath(player.previousPosition, player.position);
}
...
function payTaxToVacation(room, player, tile) {
  const amount = taxAmountFor(room, player, tile);
  if (!amount) return;
  player.cash -= amount;
...
function bankrupt(room, player) {
  ...
  if (room.pendingPurchase?.playerId === player.id) room.pendingPurchase = null;
  if (room.auction) {
    room.auction.active = room.auction.active.filter((idValue) => idValue !== player.id);
    if (room.auction.highBidderId === player.id) {
      room.auction.highBidderId = null;
      room.auction.highBid = 0;
    }
    if (room.auction.active.length <= 1) finishAuction(room);
  }
...
function drawCard(room, player, deckName) {
  ...
  if (typeof card.moveBy === "number") {
    setPlayerPosition(player, player.position + card.moveBy);
...
function scheduleAuctionTimer(room) {
  ...
  if (!room.auction?.endsAt) return;
  const delay = Math.max(0, room.auction.endsAt - Date.now());
  auctionTimers.set(room.id, setTimeout(() => {
    if (!room.auction) return;
    addLog(room, `Auction timer ended for ${BOARD[room.auction.tileId].name}.`, "auction");
    finishAuction(room);
    emitRoom(room);
  }, delay));
}
...
function startAuction(room, player, tileId) {
  ...
  addLog(room, `${player.name} sent ${tile.name} to auction. Everyone has 30 seconds to bid or pass.`, "auction");
}
...
async function api(req, res) {
  ...
  try {
    ...
    if (req.method === "POST" && url.pathname === "/api/rooms") {
      ...
      let playerId = null;
      if (body.playerName) {
        const player = addPlayer(room, body.playerName, body.playerColor);
        playerId = player.id;
      }
      emitLobby();
    ...
    const validActions = new Set([
      ...
      "build", "sellbuilding", "mortgage", "unmortgage", "trade", "trade-response", "bankrupt"
    ]);
    ...
    if (action === "join") {
      ...
      const player = addPlayer(room, body.name, body.playerColor);
    ...
    if (action === "roll") {
      ...
      if (player.cash < 0) throw new Error("Clear your negative balance with trades or mortgages before rolling.");
      ...
      if (player.jailTurns > 0) {
        if (player.jailCards > 0 && body.useJailCard) {
          ...
          if (player.cash < 0) {
            room.turnMessage = `${player.name} must clear ${moneyValue(Math.abs(player.cash))} debt before rolling.`;
            emitRoom(room);
      ...
      room.currentRoll = { rolledBy: player.id, dice: [d1, d2], total, canRollAgain: rolledDouble, doubles: rolledDouble, at: Date.now() };
      addEvent(room, "roll", `${player.name} rolled ${d1} + ${d2}${rolledDouble ? " and got doubles." : "."}`, { playerId: player.id, dice: [d1, d2], doubles: rolledDouble });
      ...
      setPlayerPosition(player, player.position + total);
      if (player.position < oldPosition) {
        player.cash += PASS_GO;
        addLog(room, `${player.name} passed Launch Square and collected $${PASS_GO}.`, "money");
      }
      resolveLanding(room, player, total);
      if (!rolledDouble) room.currentRoll.canRollAgain = false;
      maybeAutoAdvanceTurn(room);
    }
    ...
    if (action === "bid") {
      const player = findPlayer(room, body.playerId);
      if (player.bankrupt) throw new Error("Bankrupt players cannot bid.");
      const auction = room.auction;
      ...
      auction.endsAt = Date.now() + 20_000;
      scheduleAuctionTimer(room);
      addLog(room, `${player.name} bid $${amount}. Auction timer reset to 20 seconds.`, "auction");
    }
    ...
    if (action === "passbid") {
      ...
      if (!auction.active.includes(player.id)) throw new Error("You already passed this auction.");
      ...
      if (auction.highBidderId) {
        ...
        if (remaining.length === 0) finishAuction(room);
      } else if (auction.active.length === 0) {
        finishAuction(room);
      }
    }
    ...
    if (action === "build") {
      ...
      prop.houses += 1;
      addEvent(room, "build", `${player.name} built ${prop.houses === 5 ? "a hotel" : "a house"} on ${tile.name}.`, { playerId: player.id, tileId: tile.id, houses: prop.houses });
    }
    ...
    if (action === "trade") {
      ...
      if (trade.offerCash > Math.max(0, from.cash)) throw new Error("Offer cash is too high.");
      if (trade.requestCash > Math.max(0, to.cash)) throw new Error("Requested cash is too high.");
    ...
    if (action === "trade-response") {
      ...
      if (body.accept) {
        ...
        if (Math.max(0, from.cash) < trade.offerCash || Math.max(0, to.cash) < trade.requestCash) throw new Error("Cash changed; trade cannot complete.");
        trade.offerTiles.forEach((tileId) => {
